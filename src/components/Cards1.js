...
return (
    <>
        <div className='second-row-card container '>
            {
                data.filter((item) => item.category === "Bags").map((item, index)=>(index<4&&<Card key={index} style={{ width: '18rem' }} className='mx-1'>
                    <Card.Img variant="top " src={item.imageUrl} />
                    <Card.Body>
                        <Card.Title className='face1' id='face11'>{item.description }</Card.Title>
                        <Card.Text className='rs' id="rs11">RS.{item.price}.00</Card.Text>
                        <Button className='' variant="" id="btn11">Add to card</Button>
                    </Card.Body>
...
<NavLink to="/viewall"><button className='viewallbtn'>View All</button></NavLink>

<div className='umsteal1-topsell-newarrivals'>
    
    <div ><NavLink to="/UMSTEALS1">
            <img src={UMSTEALS1} alt='umsteals1' /> 
        </NavLink></div>
    
        <div ><NavLink to="/TOPSELLERS2">
            <img src={TOPSELLERS2} alt='topseller' /> 
        </NavLink></div>

        <div ><NavLink to="/NEWARRIVALS3">
